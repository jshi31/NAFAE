...
def position_encoding_general(n_position, d_pos_vec):
    '''
    init the sinusoid position encoding table for general use
    :param n_position: position number, which means the number of word vectors in a sentence.
    :param d_pos_vec: the dimension of position vector
    '''
    # keep dim 0 for padding token position encoding zero vector
    position_enc = np.array([
        [pos / np.power(10000, 2 * (j // 2) / d_pos_vec) for j in range(d_pos_vec)]
